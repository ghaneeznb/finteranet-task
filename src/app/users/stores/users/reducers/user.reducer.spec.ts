...
describe('UsersReducer', () => {
    ...
    describe('getAllUsersFinished action', () => {
        const action = GetUsersSuccessAction({ payload: [mockUserData] });
        const state = UsersReducer(initialUsersState, action);
        it('should  "items" have value ', () => {
            expect(state.items).toEqual([mockUserData]);
        });
    });

    describe('getAllUsersFailure action', () => {
        const action = GetUsersFailureAction({ message: 'failed' });
        const state = UsersReducer(initialUsersState, action);
